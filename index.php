...
if (@$_POST['login']) {
  ...
  $admin = mysqli_query($conn,"select * from admin where username = '$npm' && password = '$sandi' ");
  $user = mysqli_query($conn,"select * from user where NPM = '$npm' && sandi = '$sandi' ");
  $superadmin = mysqli_query($conn,"select * from superadmin where iduser = '$npm' && pass = '$sandi' ");
  ...
  } else if (mysqli_num_rows($superadmin)>0) {
    $_SESSION['iduser'] = "$npm";
    header("location: superadmin/index.php");
  }  else {
